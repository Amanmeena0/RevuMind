...
ASPECT_KEYWORDS = ["battery", "charger", "display", "screen", "camera", "price", "sound", "audio", "shipping", "delivery", "service"]
...
def extract_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "entity_density" not in df.columns:
        logger.info("entity_density not found in columns. Computing capitalization-based fallback...")
        ...
        df["entity_density"] = df["clean_review_text"].astype(str).apply(count_capitalized_entities)
    ...
    if "is_topic_assigned" not in df.columns:
        ...
        df["is_topic_assigned"] = df["clean_review_text"].str.contains("|".join(ASPECT_KEYWORDS), case=False).astype(int)
...
def train_helpfulness_model(
    ...
    if not os.path.exists(data_path):
        raise FileNotFoundError(f"Cleaned reviews not found at {data_path}. Please run preprocess.py first.")
    ...
    if len(df_filtered) < 100:
        logger.warning(f"Only {len(df_filtered)} reviews found with exposure >= {min_exposure_votes}.")
    ...
    df_filtered["target_score"] = df_filtered["HelpfulnessNumerator"] / df_filtered["HelpfulnessDenominator"]
    ...
    feature_cols = [
        ...
        "rating_normalized", "time_delta_days",
        "sentiment_polarity", "sentiment_confidence",
        "entity_density", "aspect_density", "is_topic_assigned"
    ]
    ...
    if XGB_AVAILABLE:
        ...
        logger.warning("XGBoost package not available. Falling back to Scikit-Learn GradientBoostingRegressor...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="models/helpfulness/weights", help="Directory to save model weights")
    parser.add_argument("--min_votes", type=int, default=5, help="Minimum exposure votes required for target calculations")

...
class ReviewTopicModeler:
    ...
    def _initialize_models(self):
        ...
        if SENTENCE_TRANSFORMERS_AVAILABLE:
            ...
            logger.warning(
                "sentence-transformers package not found. Will use TF-IDF features as baseline embeddings."
            )
        ...
        if UMAP_AVAILABLE and HDBSCAN_AVAILABLE:
            ...
            logger.warning(
                "UMAP/HDBSCAN packages not available. Falling back to Scikit-Learn PCA + MiniBatchKMeans..."
            )
            self.dim_reducer = PCA(n_components=5, random_state=self.random_state)
            self.clusterer = MiniBatchKMeans(
                n_clusters=self.num_topics, random_state=self.random_state
            )
    ...
    def _calculate_c_tfidf(
        ...
        for idx, row in grouped_docs.iterrows():
            ...
            keywords_with_scores = [
                (feature_names[i], float(scores[i])) for i in sorted_indices if scores[i] > 0
            ]
            ...
            results.append(
                {
                    ...
                    "topic_name": (
                        f"Topic {label_int}: " + ", ".join([k[0] for k in keywords[:3]])
                        if keywords
                        else f"Topic {label_int}"
                    ),
                }
            )
...
if __name__ == "__main__":
    ...
    labels, details = modeler.predict(
        ["I love the battery length and fast charging screen.", "Customer support was very rude."]
    )

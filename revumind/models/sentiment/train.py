...
def train_sentiment_model(
    ...
    if not os.path.exists(data_path):
        raise FileNotFoundError(
            f"Cleaned dataset not found at {data_path}. Please run preprocess.py first."
        )
    ...
    logger.info(
        f"Label distribution: Negative (0): {label_counts.get(0, 0)}, Neutral (1): {label_counts.get(1, 0)}, Positive (2): {label_counts.get(2, 0)}"
    )
    ...
    logger.info(
        f"Final Validation Results: Accuracy={eval_results['eval_accuracy']:.4f}, Macro F1={eval_results['eval_f1_macro']:.4f}"
    )
    ...
    report = classification_report(
        val_labels, pred_labels, target_names=["Negative", "Neutral", "Positive"]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="models/sentiment/weights",

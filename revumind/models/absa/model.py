...
class AspectSentimentAnalyzer:
    ...
    def __init__(
        self, model_name: str = "yangheng/deberta-v3-base-absa-v1.1", use_deberta: bool = True
    ):
    ...
    def _initialize_pipeline(self):
        ...
        if self.use_deberta:
            try:
                ...
                logger.warning(
                    f"Failed to load DeBERTa ABSA: {e}. Falling back to lexicon method."
                )
    ...
    def _deberta_absa(self, text: str, aspect: str) -> Dict[str, Any]:
        ...
        inputs = self.tokenizer(
            text, aspect, truncation=True, padding=True, max_length=512, return_tensors="pt"
        )
...
if __name__ == "__main__":
    ...
    for item in aspect_sentiments:
        print(
            f"  {item['aspect'].capitalize()} -> {item['sentiment_label'].upper()} (Confidence: {item['confidence']:.2f}, Polarity Score: {item['score']})"
        )

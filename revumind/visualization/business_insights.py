...
def analyse_complaints(df: pd.DataFrame, text_col: str = "review_text") -> pd.DataFrame:
    ...
    print(f"\n  TOP COMPLAINT: {top}")
    print(
        f"  Insight: Prioritise '{top}' — highest severity score signals "
...
def extract_trending_topics(
    ...
    def prep(t):
        t = t.lower()
        t = re.sub(r"[^\w\s]", " ", t)
        return " ".join(w for w in t.split() if w not in STOP_WORDS and len(w) > 3)

    corpus = df[text_col].fillna("").apply(prep).tolist()
    vec = CountVectorizer(max_features=500, ngram_range=(1, 2), min_df=3, max_df=0.9)
    ...
    monthly_pivot = monthly.pivot(index="month", columns="topic_name", values="count").fillna(0)
    ...
    if len(months) >= 6:
        ...
        print(
            f"  Insight: Monitor '{trending_up}' — accelerating volume "
...
def track_defect_rates(df: pd.DataFrame, text_col: str = "review_text") -> pd.DataFrame:
    ...
    df = df.copy()
    df["is_defect"] = (
        df[text_col].str.lower().str.contains(pattern, regex=True, na=False).astype(int)
    )

    defect_by_product_month = (
        df.groupby(["product", "month"])
        .agg(total_reviews=("review_text", "count"), defect_mentions=("is_defect", "sum"))
        .reset_index()
    )
    defect_by_product_month["defect_rate_pct"] = (
        defect_by_product_month["defect_mentions"] / defect_by_product_month["total_reviews"] * 100
    ).round(2)
    ...
    critical = defect_by_product_month[defect_by_product_month["alert"] == "🔴 CRITICAL"]
    if len(critical):
        ...
        print(
            f"  Insight: Cross-reference production batch from {row['month']} "
...
def analyse_aspect_sentiment(df: pd.DataFrame, text_col: str = "review_text") -> pd.DataFrame:
    ...
    for aspect, keywords in ASPECTS.items():
        ...
        pos_count = subset.apply(lambda t: any(w in t for w in POS_WORDS)).sum()
        neg_count = subset.apply(lambda t: any(w in t for w in NEG_WORDS)).sum()
    ...
    if len(worst):
        ...
        print(
            f"  Insight: {row['aspect']} is the most-discussed negative feature — "
...
def detect_suspicious_reviews(df: pd.DataFrame, text_col: str = "review_text") -> pd.DataFrame:
    ...
    unverified_5star = (~df["verified"]) & (df["star_rating"] == 5) & (df["helpful_votes"] == 0)
    ...
    duplicates = df[text_col].str.lower().str.strip().isin(text_counts[text_counts > 2].index)
    flags[duplicates] += 2
    ...
    print(
        f"  Insight: {suspicious_pct:.0f}% suspected fake — "
...
def plot_executive_dashboard(
    ...
    fig.text(
        0.5,
        0.96,
        "Product Review Intelligence — Executive Dashboard",
        ha="center",
        fontsize=19,
        fontweight="bold",
        color="#1A1A1A",
    ...
    kpi_ax.set_title("Key Performance Indicators", fontweight="bold", fontsize=12, pad=8)

    # ── Panel 1: Top complaints ───────────────────────────────────────────────
    ax1 = fig.add_subplot(gs[1, 0])
    top_c = complaints.head(7)
    colors_c = [PALETTE[1] if s > 200 else PALETTE[2] for s in top_c["severity_score"]]
    bars = ax1.barh(
        top_c["complaint"][::-1],
        top_c["severity_score"][::-1],
        color=colors_c[::-1],
        alpha=0.85,
        edgecolor="white",
    )
    ax1.bar_label(
        bars, labels=[f"{v:.0f}" for v in top_c["severity_score"][::-1]], padding=3, fontsize=8
    )
    ...
    if len(prod0) > 0:
        ...
        colors_d = [
            PALETTE[1] if r > 10 else PALETTE[2] if r > 5 else PALETTE[0] for r in rates_plot
        ]
        ax2.bar(
            range(len(months_plot)),
            rates_plot,
            color=colors_d,
            alpha=0.85,
            edgecolor="white",
        ...
        ax2.axhline(5, color=PALETTE[2], linewidth=1.2, linestyle="--", label="5% Warning")
        ax2.axhline(10, color=PALETTE[1], linewidth=1.2, linestyle="--", label="10% Critical")
    ...
    ax3.set_title("Aspect Sentiment Map\n(size = mention count)", fontweight="bold", fontsize=11)
    # Quadrant labels
    ax3.text(
        0.72,
        0.95,
    ...
    for i, complaint in enumerate(top3_complaints):
        pattern = "|".join(re.escape(kw) for kw in COMPLAINT_KEYWORDS[complaint])
    ...
    ax6.set_title("Complaint Rate by Product (%)", fontweight="bold", fontsize=11)
...
def plot_complaint_deep_dive(
    ...
    for _, row in complaints.iterrows():
        ax.annotate(
            row["complaint"][:15],
            (row["mention_count"], row["severity_score"]),
            xytext=(4, 2),
            textcoords="offset points",
            fontsize=7.5,
        )
    ...
    for complaint, keywords in list(COMPLAINT_KEYWORDS.items())[:5]:
        ...
        for star in [1, 2, 3]:
            ...
            pct = (
                sub["review_text"].str.lower().str.contains(pattern, regex=True, na=False).mean()
                * 100
            )
            complaint_by_star.append({"complaint": complaint[:14], "star": f"{star}★", "pct": pct})
    cb_df = pd.DataFrame(complaint_by_star)
    if len(cb_df):
        pivot = cb_df.pivot(index="complaint", columns="star", values="pct").fillna(0)
        pivot.plot(kind="bar", ax=ax2, color=PALETTE[:3], alpha=0.85, edgecolor="white", width=0.7)
    ...
    pattern_top = "|".join(re.escape(k) for k in COMPLAINT_KEYWORDS[top_complaint])
    df2 = df.copy()
    df2["top_complaint"] = (
        df2["review_text"].str.lower().str.contains(pattern_top, regex=True, na=False).astype(int)
    )
    monthly_trend = (
        df2.groupby("month")
        .agg(
    ...
    ax3.fill_between(range(len(months_t)), monthly_trend["rate"], alpha=0.15, color=PALETTE[1])
    ...
    plt.suptitle("Complaint Analysis — Deep Dive", fontsize=14, fontweight="bold", y=1.02)
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    print(f"\n  Top 5 complaints:")
    print(
        complaints[["complaint", "mention_count", "pct_neg_reviews", "severity_score"]]
        .head(5)
    ...
    print(f"\n  Aspect sentiment:")
    print(
        aspect_sent[["aspect", "mentions", "pos_pct", "neg_pct", "net_sentiment"]].to_string(
            index=False
        )
    )
    ...
    plot_executive_dashboard(
        df, complaints, defect_rates, aspect_sent, kpis, "executive_dashboard.png"
    )
    ...
    if len(critical_defect):
        ...
        print(
            f"     CRITICAL: {row['product']} in {row['month']}: "
            f"{row['defect_rate_pct']:.1f}% defect rate"
        )
    ...
    print(
        f"     → Score is {'positive — highlight in marketing' if kpis['nps_proxy']>20 else 'below target — prioritise complaint resolution'}"
    )
    print("=" * 62 + "\n")

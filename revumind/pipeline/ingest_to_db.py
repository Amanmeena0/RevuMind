...
def ingest_reviews_to_db(
    ...
    logger.info(f"Loading raw reviews from {raw_data_path}...")
    # Load target columns
    target_cols = [
        "ProductId", "UserId", "ProfileName", "Score", "Summary", "Text",
        "HelpfulnessNumerator", "HelpfulnessDenominator", "Time"
    ]
    df = pd.read_csv(raw_data_path, usecols=lambda col: col in target_cols, nrows=sample_size * 2)
    ...
    try:
        for idx, row in df.iterrows():
            ...
            embedding = models.ReviewEmbedding(
                review_id=review.id,
                embedding=analysis["embedding"] # stored as list (handles pgvector list or pickle serialize)
            )
            db.add(embedding)
        ...
        logger.info("=========================================")
        logger.info(f"Successfully Ingested: Reviews={count_reviews}, Entities={count_entities}, Aspect Sentiments={count_aspects}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--min_votes", type=int, default=0, help="Minimum helpfulness votes filter")

...
class RevuMindInferenceEngine:
    ...
    def __init__(
        ...
        if os.path.exists(topics_model_path):
            ...
            logger.warning(
                f"Fitted Topic Model not found at {topics_model_path}. Topic modeling will run in zero-shot dummy mode."
            )
    ...
    def _load_helpfulness_model(self):
        ...
        if os.path.exists(model_path) and os.path.exists(scaler_path):
            ...
            logger.warning(
                "Fitted Helpfulness model not found. Predictions will return a fallback placeholder."
            )

    def analyze_single_review(
        self, raw_text: str, rating: int, review_time_unix: int = None
    ) -> Dict[str, Any]:
        """
        ...
        aspect_candidates = [
            ent["text"] for ent in entities if ent["label"] in ["FEATURE", "PRODUCT"]
        ]

        # 4. Aspect-Based Sentiment Analysis
        aspect_sentiments = self.absa_analyzer.analyze_review_aspects(
            clean_text, aspects=aspect_candidates
        )
    ...
    def analyze_batch(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        positive_reviews = df_results[df_results["overall_sentiment"] == "positive"][
            "clean_review_text"
        ].tolist()[:10]
        negative_reviews = df_results[df_results["overall_sentiment"] == "negative"][
            "clean_review_text"
        ].tolist()[:10]
        ...
        insights = {
            ...
            "summaries": {
                "positive_cohort_summary": (
                    self.summarizer.generate_summary(positive_reviews)
                    if positive_reviews
                    else "No positive reviews to summarize."
                ),
                "negative_cohort_summary": (
                    self.summarizer.generate_summary(negative_reviews)
                    if negative_reviews
                    else "No negative reviews to summarize."
                ),
            },
        ...
        if not insights["business_recommendations"]:
            insights["business_recommendations"].append(
                "Monitor product reception to gather performance points."
            )
...
if __name__ == "__main__":
    ...
    print(
        f"Overall Sentiment: {result['overall_sentiment'].upper()} (Confidence: {result['sentiment_confidence']})"
    )

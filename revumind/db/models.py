...
from sqlalchemy import (
    ...
    Text, JSON, PickleType, CheckConstraint
)
...
class Review(Base):
    ...
    embedding = relationship("ReviewEmbedding", back_populates="review", uselist=False, cascade="all, delete-orphan")
...
class Summary(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    product_id = Column(String(50), ForeignKey("products.id", ondelete="CASCADE"), index=True)
    cohort_type = Column(String(50), nullable=False) # e.g. 'all', 'positive', 'negative', 'topic_3'
